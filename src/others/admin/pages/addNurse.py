...
def check_employee_no_exists(employee_no):
    try:
        ...
        check_query = "SELECT COUNT(*) FROM nurse WHERE emp_no = %s"
...
def check_email_availability(email):
    try:
        ...
        check_query = "SELECT COUNT(*) FROM nurse WHERE ohc_mail = %s"
...
def save_ohc_mail_and_password(emp_no, ohc_mail, password):
    try:
        ...
        # SQL query to update OHC Mail and password for the nurse
        update_query = """
        UPDATE nurse SET ohc_mail = %s, password = %s WHERE emp_no = %s
        """
